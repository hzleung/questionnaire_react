 ...
 * @LastEditors: admin
 * @LastEditTime: 2019-10-11 16:46:16
 */
...
export default async ({
  ...
  imgUrl = '',
  openid = parse(window.location.search).openid || local.get(Constants.openid),
  ...
  if (openid === undefined && isNeedLogin && process.env.NODE_ENV === 'production') {
    ...
    local.set(Constants.openid, openid);
  ...
  wx.ready(() => {
    ...
    getLocation(res => {
      ...
      local.set(Constants.latitude, res.latitude);
      local.set(Constants.longitude, res.longitude);
    });
  });
  return true;
};
