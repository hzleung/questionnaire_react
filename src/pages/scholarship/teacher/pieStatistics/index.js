...
class ScholarshipPieStatistics extends React.Component {

  componentDidMount() {
    const { dispatch, location:{ query:{level} } } = this.props;
    dispatch({
        type:'scholarshipTeacher/querySchoolTimeData',
        level
    });
  }
  ...
  goToDetial = item => {
    ...
    router.push({
      pathname: '/scholarship/teacher/detail/index',
        query:{
            level,
  ...
  handleSchoolTimeChange = (schoolTime) => {
    ...
    const { dispatch, location:{ query:{level} } } = this.props;
    dispatch({
        type:'scholarshipTeacher/queryScholarshipStatistics',
        level,
        schoolTime:time
    });
    dispatch({
  ...
  getOption = () => {
    const { scholarshipStatistics=[] } = this.props;
    let maleTotalCount = 0;
    let femaleTotalCount = 0;
    scholarshipStatistics.forEach(item=>{
  ...
  render() {
    const { schoolTimeData, selectedSchoolTime, scholarshipStatistics } = this.props;
    return (
      <div
        style={{
      ...
      >
        ...
        <div className={styles.itemBox}>
          ...
          <div className={styles.detialBox}>
            {
              scholarshipStatistics && scholarshipStatistics.length > 0 ?
              scholarshipStatistics.map((item, index) => {
                return (
                  <div
                  ...
                  >
                    ...
                    {index !== scholarshipStatistics.length - 1 && <div className={styles.scaleBox}><div className={styles.scale} /></div>}
                  </div>
                );
...
export default connect((state)=>{return {...state.scholarshipTeacher};})(ScholarshipPieStatistics);
