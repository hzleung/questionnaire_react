...
class GrantPieStatistics extends React.Component {
  ...
  componentDidMount() {
    ...
    dispatch({
        type:'grantTeacher/querySchoolTimeData',
        level
    });
  }
  ...
  goToDetial = item => {
    ...
    router.push({
      pathname: '/grant/teacher/detail/index',
        query:{
            level,
  ...
  handleSchoolTimeChange = (schoolTime) => {
    ...
    const { dispatch, location:{ query:{level} } } = this.props;
    dispatch({
        type:'grantTeacher/queryStatistics',
        level,
        schoolTime:time
    });
    dispatch({
...
export default connect((state)=>{return {...state.grantTeacher};})(GrantPieStatistics);
