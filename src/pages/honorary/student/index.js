...
class index extends React.Component {
  ...
  componentDidMount() {
    ...
    this.props.dispatch({
      type: 'honorary_student/fetchHonoraryInfo',
      payload: { yhbh: userId },
      search: search,
    });
  }
      ...
      <div style={{ background: 'rgba(242,244,247,1);' }}>
        {
          (mock && mock.length == 0 || !mock) ?
            ...
            <div>
              ...
              <div className = {styles.chufen}>我的荣誉称号</div>
              <div className={styles.itemBox}>
                {
                  mock.map((item) => {
                    ...
                    return (
                      <Link
                            to={{
                            pathname: "/honorary/student/detail/index",
...
function mapStateToProps(state) {
  const { routing,login, honorary_student } = state;
  return {
      user: login.user,
      ...honorary_student
  };
